...
enum class DateFilter(val value:String,val dayCode:Int) {
    ...
    open fun getStartDatetime(): String? {
        return when (this) {
            ...
            LAST_MONTH -> DateTimeUtils.getMonthFirstDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,1) + " 00:00:00"
            LAST_3_MONTHS -> DateTimeUtils.getMonthFirstDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,3) + " 00:00:00"
            LAST_6_MONTHS -> DateTimeUtils.getMonthFirstDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,6) + " 00:00:00"
            else -> null
        }
    }
    ...
    open fun getEndDatetime(): String? {
        return when (this) {
            ...
            LAST_MONTH -> DateTimeUtils.getMonthLastDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,1) + " 23:59:59"
            LAST_3_MONTHS -> DateTimeUtils.getMonthLastDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,3) + " 23:59:59"
            LAST_6_MONTHS -> DateTimeUtils.getMonthLastDate(DateTimeFormat.yyyy_MM_dd, DateTimeZone.IST,6) + " 23:59:59"
            else -> null
        }
    }
}
...
object DateTimeUtils {
    ...
    fun getMonthFirstDate(dFormat: DateTimeFormat, zone: DateTimeZone, n:Int): String? {
    ...
    fun getMonthLastDate(dFormat: DateTimeFormat, zone: DateTimeZone, n:Int): String? {
        val aCalendar = Calendar.getInstance()
        aCalendar.add(Calendar.MONTH, n)
        aCalendar[Calendar.DATE] = 1
    ...
    fun getDateDifferenceInDays(
        ...
        val sdf = SimpleDateFormat(DateTimeFormat.yyyy_MM_dd_HH_mm_ss.value)
        val differenceInTime = sdf.parse(endDate).time - sdf.parse(startDate).time
        val days = TimeUnit.DAYS.convert(differenceInTime, TimeUnit.MILLISECONDS)
        return days.toInt()
    }
